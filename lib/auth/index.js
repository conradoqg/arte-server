...
class Auth {
    ...
    async createToken(username, password) {
        ...
        if (userFound && (username == userFound.username && bcrypt.compareSync(password, userFound.password))) {
            token = jwt.sign(Auth.createCredential(userFound.username, userFound.roles), JWT_SECRET, { expiresIn: '90d' });
        }
            ...
            if (this.ldapConfig) {
                const tries = this.ldapConfig.map(async (config) => {
                    try {
                        ...
                        token = jwt.sign(Auth.createCredential(userFound ? userFound.username : username, userFound ? userFound.roles : ['guest']), JWT_SECRET, { expiresIn: '90d' });
                    } catch (ex) {
