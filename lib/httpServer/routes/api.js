...
module.exports = (dbService, bucketService, artifactService, webhookService, authService, metricService) => {
    ...
    router.get('/healthcheck', (req, res) => {
        if (!req.accepts('json')) throw new HTTPError(406, 'Not Acceptable');
        res.status(200).json({ healthy: dbService.isHealthy() });
    });
    ...
    router.get('/buckets/:bucketName/artifacts/:artifactName/:version?', authService.createMiddleware(false), async (req, res) => {
        ...
        if (req.accepts('zip')) {
            artifactService.incrementArtifactCounter(artifact, 'downloads');
    ...
    router.get('/admin/metrics', authService.createMiddleware(), async (req, res) => {
        if (!req.accepts('json')) throw new HTTPError(406, 'Not Acceptable');

        res.status(200).json(await metricService.getMetrics(req, req.query.reset));
    });
