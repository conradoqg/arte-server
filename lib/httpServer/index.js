...
class HTTPServer {
    constructor(dbService, bucketService, artifactService) {
        ...
        // /buckets/bucket1/artifacts/artifact1 -> Get all artifacts for a given name
        // /buckets/bucket1/artifacts/artifact1/latest -> Get the latest artifact version for a given metadata
        // /buckets/bucket1/artifacts/artifact1/1.0 -> Get all artifacts of version 1 for a given metadata
        // /buckets/bucket1/artifacts/artifact1/1.0/latest -> Get the latest artifact of version 1 for a given metadata
        ...
        this.app.get('/buckets/:bucketName/artifacts/:artifactName/:version', async (req, res) => {
            ...
            // When a zip is requested always return the first artifact found since it was not specified explicity wich version, the user can order the artifact selection by using the metadata
            if (req.accepts('zip')) {
