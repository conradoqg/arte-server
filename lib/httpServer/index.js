...
class HTTPServer {
    constructor(dbService, bucketService, artifactService, webhookService, authService) {
        ...
        this.app.put('/buckets/:bucketName/artifacts/:artifactName/:version?', authService.createMiddleware(false), upload, async (req, res) => {
            ...
            const artifact = await artifactService.createOrUpdateArtifact(req.credential, bucket, artifactName, version, filePath, metadata);
        ...
        this.app.get('/buckets/:bucketName/artifacts/:artifactName/:version?', authService.createMiddleware(false), async (req, res) => {
            ...
            const artifact = await artifactService.getArtifact(req.credential, bucket, artifactName, version, metadata);
        ...
        this.app.delete('/buckets/:bucketName/artifacts/:artifactName/:version?', authService.createMiddleware(false), async (req, res) => {
            ...
            const artifact = await artifactService.removeArtifact(req.credential, bucket, artifactName, version, metadata);
        ...
        this.app.get(['/', '/artifacts/search'], authService.createMiddleware(false), async (req, res) => {
            ...
            if (req.accepts('html')) {
                ...
                const buckets = await bucketService.getBuckets(req.credential);
                const artifacts = await artifactService.findArtifacts(req.credential, bucketName, artifactName, version, metadata, partial);
                ...
                let artifacts = await artifactService.findArtifacts(req.credential, bucketName, artifactName, version, metadata, partial);
        ...
        this.app.get('/users/profile', authService.createMiddleware(), async (req, res) => {
            if (!req.accepts('html')) throw new HTTPError(406, 'Not Acceptable');
        ...
        this.app.get('/tokens/grants', authService.createMiddleware(), async (req, res) => {
            if (!req.accepts('html')) throw new HTTPError(406, 'Not Acceptable');

            const buckets = await this.bucketService.findBucketsByOwner(req.credential);
        ...
        this.app.post('/tokens/grants', authService.createMiddleware(), async (req, res) => {
            if (!req.accepts('json')) throw new HTTPError(406, 'Not Acceptable');
            ...
            const token = await authService.createGrantToken(req.credential, username, buckets, artifactsCreate, artifactUpdate, artifactRemove);
