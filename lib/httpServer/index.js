...
class HTTPServer {
    constructor(dbService, bucketService, artifactService) {
        ...
        // TODO: UI View  
        // TODO: Protect name, version and so on with slugify
        ...
        this.app.get('/buckets/:bucketName/artifacts/:artifactName/:version', async (req, res) => {
            ...
            if (req.accepts('json')) res.status(200).json(artifact.project());
            else if (req.accepts('zip')) res.status(200).download(await artifactService.getFullPath(artifact.path));
        });
