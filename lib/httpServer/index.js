...
class HTTPServer {
    constructor(dbService, bucketService, artifactService) {
        ...
        this.app.get(['/', '/artifacts/search'], async (req, res) => {
            ...
            const partial = slug(req.query.partial);
            const metadata = req.query;
            ...
            if (req.accepts('html')) {
                ...
                let artifacts = await artifactService.getArtifacts(bucketName, artifactName, version, metadata);
                res.status(200).json(artifacts.map(artifact => artifact.project()));
            }
        });
