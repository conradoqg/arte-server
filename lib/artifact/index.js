...
class Artifact {
    ...
    async createOrUpdateArtifact(credential, bucket, artifactName, version, filePath, metadata) {
        ...
        if (!await this.authService.checkCredential(credential, 'artifacts:create', { credential, bucket }) || !await this.authService.checkCredential(credential, 'artifact:update', { credential, bucket })) throw new AuthorizationError(`The user '${credential.username}' is not authorized to create artifacts or update the artifact `);
