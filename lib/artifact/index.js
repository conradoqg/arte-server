...
class Artifact {
    ...
    async storeArtifact(bucket, artifactName, version, filePath, metadata) {
        ...
        const { destinationFilename, fileSize } = await this.storageDB.storeArtifact(bucket.name, artifactName, version, filePath, metadata);

        return await this.metadataDB.createOrUpdateArtifact(bucket.name, artifactName, version, destinationFilename, fileSize, metadata);
    }
