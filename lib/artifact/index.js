...
class Artifact {
    ...
    async normalizeMetadata(metadata) {
        ...
        Object.keys(template.properties).forEach(key => {
            ...
            if (metadata[key] && template.properties[key].oneOf && !template.properties[key].oneOf.includes(metadata[key])) throw new InvalidOperationError(`The metadata '${key}' must be one of ${template.properties[key].oneOf}`);
        });
    ...
    async storeArtifact(bucket, artifactName, version, filePath, metadata) {
        ...
        if (typeof (metadata) != 'object') throw new TypeError('The metadata argument must be an object');
    ...
    async getLatestArtifact(bucket, artifactName, metadata) {
        ...
        metadata = await this.normalizeMetadata(metadata, true);
