...
class Artifact {
    ...
    async storeArtifact(bucket, artifactName, version, filePath, metadata) {
        ...
        const { destinationPath, fileSize } = await this.storageDB.storeArtifact(bucket.name, artifactName, version, filePath, metadata);

        return await this.metadataDB.createOrUpdateArtifact(bucket.name, artifactName, version, destinationPath, fileSize, metadata);
    }

    async getArtifacts(bucket, artifactName, version, metadata, normalized = true) {
        if (typeof (bucket) != 'object') throw new TypeError('The bucket argument must be an object');
        if (typeof (artifactName) != 'string') throw new TypeError('The artifactName argument must be a string');

        if (normalized) metadata = await this.normalizeMetadata(metadata);

        const artifact = await this.metadataDB.findArtifacts(bucket.name, artifactName, version, metadata);

        return artifact;
    }

    async deleteArtifacts(bucket, artifactName, version, metadata) {
        if (typeof (bucket) != 'object') throw new TypeError('The bucket argument must be an object');
        if (typeof (artifactName) != 'string') throw new TypeError('The artifactName argument must be a string');        

        const artifacts = await this.metadataDB.findArtifacts(bucket.name, artifactName, version, metadata);

        const deleteMetadataPromise = this.metadataDB.deleteArtifacts(artifacts);
        const deleteStoragePromises = this.storageDB.removeArtifacts(artifacts);

        const promises = [deleteMetadataPromise, ...deleteStoragePromises];
