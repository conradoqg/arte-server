...
class Bucket {
    ...
    async getBucket(credential, bucketName) {
        ...
        if (bucket && !await this.authService.checkCredential(credential, 'bucket:read', { credential, bucket })) throw new AuthorizationError(`The user '${credential.username}' is not authorized to read the bucket`);

        return bucket;
    }

    async findBucketsByOwner(credential) {
        return this.metadataDB.findBucketsByOwner(credential.username);
    }
    ...
    async createBucket(credential, bucketName, template) {
        ...
        if (!bucket) bucket = await this.metadataDB.createBucket(bucketName, template, credential.username);
