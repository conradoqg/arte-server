...
class MetadataDB {
    ...
    async createOrUpdateArtifact(bucketName, name, version, path, fileSize, metadata) {
        ...
        let artifact = await this.getArtifact(bucketName, name, version, metadata);

        if (!artifact) artifact = await (new this.Artifact({ bucket: bucketName, name, version, normalizedVersion: normalizeVersion(version), path, fileSize, lastUpdate: Date.now(), metadata })).save();
        else artifact.set({ lastUpdate: Date.now() }).save();
