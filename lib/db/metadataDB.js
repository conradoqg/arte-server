...
class MetadataDB {
    ...
    async findArtifacts(bucketName, name, version, metadata, partial = false) {
        let sortMap = new Map();
        ...
        if (name) query.name = (partial ? new RegExp(name, 'i') : name);
    ...
    deleteArtifacts(artifacts) {
        const idsToDelete = artifacts.map(artifact => ObjectId(artifact.id));
