...
class MetadataDB {
    ...
    async connect() {
        ...
        this.Artifact = this.connection.model('File', new mongoose.Schema({ bucket: 'string', name: 'string', version: 'string', normalizedVersion: 'string', path: 'string', metadata: 'mixed' }));
        return;
    }
    ...
    async createOrUpdateArtifact(bucketName, name, version, path, metadata) {
        if (typeof (bucketName) != 'string') throw new TypeError('The bucketName argument must be a string');

        let artifact = await this.findArtifact(bucketName, name, version, metadata);

        if (!artifact) artifact = await (new this.Artifact({ bucket: bucketName, name, version, normalizedVersion: normalizeVersion(version), path, metadata })).save();
