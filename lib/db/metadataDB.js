...
class MetadataDB {
    ...
    async createArtifact(bucketName, name, version, path, fileSize, metadata) {
        ...
        return (new this.Artifact({ bucket: bucketName, name, version, normalizedVersion: normalizeVersion(version), path, fileSize, lastUpdate: Date.now(), uploads: 1, downloads: 0, metadata })).save();
    }

    async updateArtifact(artifact, fileSize) {
        return await artifact.set({ fileSize, updatedAt: Date.now(), uploads: artifact.uploads + 1 }).save();
    }
    ...
    async incrementArtifactCounter(artifact, counter) {
        return this.Artifact.findOneAndUpdate({ _id: artifact._id }, { $inc: { [counter]: 1 } }).exec();
    }
